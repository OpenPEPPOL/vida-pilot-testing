...
File `NW-HP-002.SE-SK.PeppolBIS.xml` implements a testing scenario for a **Reverse Charge** transaction of services, where the VAT rate indicated is `0%` because the tax responsibility is shifted to the buyer.
...
* C2 generates the source invoice based on the received data (file `NW-HP-002.SE-SK.PeppolBIS.xml`).
* C2 validates the generated source invoice against applicable schematrons (files `supporting-files/NW-HP-002.SE-SK.PeppolBIS-validation.xml`)
* C2 generates the supply-side TDD (file `sample-results/NW-HP-002.SE-SK.PeppolBIS.TDD-C2.xml`)
* C2 validates the generated supply-side TDD against applicable schematrons (files `supporting-files/NW-HP-002.SE-SK.PeppolBIS.TDD-C2.validation.xml`)
...
* C3 receives inbound invoice from C2 (file `NW-HP-002.SE-SK.PeppolBIS.xml`).
* C3 validates the received invoice against applicable schematrons (files `supporting-files/NW-HP-002.SE-SK.PeppolBIS-validation.xml`)
* C3 generates the buy-side TDD (file `sample-results/NW-HP-002.SE-SK.PeppolBIS.TDD-C3.xml`)
* C3 validates the generated buy-side TDD against applicable schematrons (files `supporting-files/NW-HP-002.SE-SK.PeppolBIS.TDD-C3.validation.xml`)
...
* C5 receives inbound TDD from C2/C3, files
  * `sample-results/NW-HP-002.SE-SK.PeppolBIS.TDD-C2.xml` resp.
  * `sample-results/NW-HP-002.SE-SK.PeppolBIS.TDD-C3.xml`.
* C5 validates the received TDD against applicable schematrons, files
  * `supporting-files/NW-HP-002.SE-SK.PeppolBIS.TDD-C2.validation.xml` and
  * `supporting-files/NW-HP-002.SE-SK.PeppolBIS.TDD-C3.validation.xml`.

### Message-level status

* C3 sends positive MLS to C2 (file `sample-results/NW-HP-002.SE-SK.PeppolBIS.MLS-C3.xml`)
* C5A sends positive MLS to C2 (file `sample-results/NW-HP-002.SE-SK.PeppolBIS.MLS-C5A.xml`)
* C5B sends positive MLS to C3 (file `sample-results/NW-HP-002.SE-SK.PeppolBIS.MLS-C5B.xml`)
