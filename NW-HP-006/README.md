...
Files `NW-HP-006.AA-BB.PeppolBIS.xml` implement a basic testing scenario for an **Intra-Community Supply (ICS)** of goods, where the VAT rate indicated is 0% because the tax responsibility is shifted to the buyer and the invoice contains PDF attachments with supplemental information about the goods invoiced.
...
This test scenario serves to execute the basic **network "happy path"** of a **cross-border invoice** including attachments.
...
The invoice uses the Peppol BIS Billing 3.0 profile and contains PDF attachments.
...
* C2 generates the source invoice based on the received data (file `NW-HP-006.AA-BB.PeppolBIS.xml`).
* C2 validates the generated source invoice against applicable schematrons (files `supporting-files/NW-HP-006.AA-BB.PeppolBIS-validation.xml`)
* C2 generates the supply-side TDD (file `sample-results/NW-HP-006.AA-BB.PeppolBIS.TDD-C2.xml`).  
The TDD does not include any information about the attachments of the invoice.
* C2 validates the generated supply-side TDD against applicable schematrons (files `supporting-files/NW-HP-006.AA-BB.PeppolBIS.TDD-C2.validation.xml`)
...
* C3 receives inbound invoice from C2 (file `NW-HP-006.AA-BB.PeppolBIS.xml`).
* C3 validates the received invoice against applicable schematrons (files `supporting-files/NW-HP-006.AA-BB.PeppolBIS-validation.xml`)
* C3 generates the buy-side TDD (file `sample-results/NW-HP-006.AA-BB.PeppolBIS.TDD-C3.xml`)  
The TDD does not include any information about the attachments of the invoice.
* C3 validates the generated buy-side TDD against applicable schematrons (files `supporting-files/NW-HP-006.AA-BB.PeppolBIS.TDD-C3.validation.xml`)
...
* C5 receives inbound TDD from C2/C3, files
  * `sample-results/NW-HP-006.AA-BB.PeppolBIS.TDD-C2.xml` resp.
  * `sample-results/NW-HP-006.AA-BB.PeppolBIS.TDD-C3.xml`.
* C5 validates the received TDD against applicable schematrons, files
  * `supporting-files/NW-HP-006.AA-BB.PeppolBIS.TDD-C2.validation.xml` and
  * `supporting-files/NW-HP-006.AA-BB.PeppolBIS.TDD-C3.validation.xml`.

### Message-level status

* C3 sends positive MLS to C2 (file `sample-results/NW-HP-006.AA-BB.PeppolBIS.MLS-C3.xml`)
* C5A sends positive MLS to C2 (file `sample-results/NW-HP-006.AA-BB.PeppolBIS.MLS-C5A.xml`)
* C5B sends positive MLS to C3 (file `sample-results/NW-HP-006.AA-BB.PeppolBIS.MLS-C5B.xml`)

### Make results available to C1/C4/C6 (out of scope of Testing Scenario)

* Invoice receiver's side
    * C3 generates and sends invoice data available to C4, including the attachments.
    * C3 makes received invoice and TDD available to C4, including the attachments.
    * C3 makes results as confirmed available to C4
* Invoice sender's side
    * C2 makes generated invoice and TDD available to C1, including the attachments.
    * C2 makes received outcome of transmissions available to C1.
* Tax Administration's side
    * C5 makes received TDD available to C6. Invoice attachments are not visible to C6.
